...
func (app *application) Login(w http.ResponseWriter, r *http.Request) {
	...
	user, err := app.userModels.GetByEmail(creds.UserName)
	if err != nil {
	...
	token, err := app.tokenModels.GenerateToken(user.ID, 24*time.Hour)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
	...
	err = app.tokenModels.InsertToken(*token, *user)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
